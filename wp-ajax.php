...
if ( !class_exists( 'WPAjax' ) ) {
	class WPAjax {
		...
		function enqueue_scripts() {
			...
			wp_register_script('transition-events',WP_AJAX_PLUGIN_URL . '/js/includes/transition-events.js', array(), false, true );
			...
			wp_register_script('modernizr-custom-ajaxify', WP_AJAX_PLUGIN_URL . '/js/includes/modernizr-custom.min.js', array('jquery'), false, true );
			wp_register_script('jquery-ajaxify', WP_AJAX_PLUGIN_URL . '/js/includes/jquery.ajaxify.js', array('transition-events', 'jquery','jquery-address','jquery-form','jquery-imagesloaded','modernizr-custom-ajaxify'), false, true );
			wp_register_script(WP_AJAX_SCRIPT_UID, WP_AJAX_PLUGIN_URL . '/js/jquery.wp-ajax.js', array('transition-events','jquery-ajaxify', 'jquery','jquery-address','jquery-form', 'jquery-imagesloaded'), false, true );
			
			if (!is_admin()) {
				wp_enqueue_script('transitionEnd');
				...
				wp_localize_script( 'jquery-wp-ajax', 'wpAjaxify', array( 'ajaxurl' => WP_AJAX_PLUGIN_URL . '/wp-ajax-client.php', 'baseurl'=> home_url().'/', 'container'=> $container, 'container_class'=> 'wp-ajax-container', 'loading_container' => $loading_container, 'loading_html' => $loading_html, 'links_selector' => $link_selector, 'loading_test_mode' => $loading_test_mode, 'loading_position_container' => $loading_container_position_selector, 'loading_position' => $loading_position, 'no_cache' => $no_cache_urls ));
